...
export function compute_probs(price, strike, volatility, time) {
  ...
  const prob_profit_call =
    1 - compute_prob_itm(price, strike + call, volatility, time);
  const put = strike * stdNormalCDF(-d2) - price * stdNormalCDF(-d1);
  const prob_profit_put =
    1 - compute_prob_itm(price, strike - put, volatility, time);
