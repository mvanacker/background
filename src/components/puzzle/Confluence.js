...
const Confluence = memo(
  ({
    ...
    useEffect(() => {
      ...
      const toleratedData = data.filter(
        (d) => lower < d.price && d.price < upper
      );
      ...
      const priceAxis = (g) =>
        g.call(
          axisLeft(priceScale).tickValues(lumps.sortedPrices).tickSizeOuter(0)
        );
      ...
      const lumpNameAxis = (g) =>
        g.call(
          axisRight(priceScale)
            .tickValues(lumps.representatives)
            .tickSizeOuter(0)
        );

      const unlumpNameAxis = (g) =>
        g.call(
          axisRight(priceScale)
      ...
      svg
        .selectAll('.price-axis .tick text')
      ...
      svg
        ...
        .call(translateAxis)
        .call(lumpNameAxis);
      svg.selectAll('.lump-name-axis text').text((_, i) => lumps.names[i]);
      ...
      svg
        ...
        .call(unlumpNameAxis);
      svg
        .selectAll('.unlump-name-axis text')
...
const Checks = ({
  ...
  <ul className="my-checks my-kreon">
    <li>{title}</li>
      ...
      return (
        <li key={id}>
          <label htmlFor={id}>
            ...
            />
            <span className="my-check-label">{key2label(key)}</span>
          </label>
        </li>
      );
    })}
