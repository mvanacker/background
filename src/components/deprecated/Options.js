...
export default withCookies(
  class Options extends Component {
      ...
      this.state = {
        ...
        form: {
          underlying: {
            ...
            hedgeOnly: ((c) => (c ? c === 'true' : true))(
              cookies.get('form-underlying-hedgeOnly')
            ),
          },
          optionPractical: {
            quantity: cookies.get('form-optionPractical-quantity') || 0.1,
            underlyingPrice:
              cookies.get('form-optionPractical-underlyingPrice') || 7340,
            strike: cookies.get('form-optionPractical-strike') || 7000,
            daysRemaining:
              cookies.get('form-optionPractical-daysRemaining') || 16,
            hoursRemaining:
              cookies.get('form-optionPractical-hoursRemaining') || 16,
            daysTransacted:
              cookies.get('form-optionPractical-daysTransacted') || 0,
            hoursTransacted:
              cookies.get('form-optionPractical-hoursTransacted') || 0,
          ...
          optionTheoretical: {
            ...
            underlyingPrice:
              cookies.get('form-optionTheoretical-underlyingPrice') || 7340,
            strike: cookies.get('form-optionTheoretical-strike') || 7000,
            volatility: cookies.get('form-optionTheoretical-volatility') || 1,
            daysRemaining:
              cookies.get('form-optionTheoretical-daysRemaining') || 1,
            hoursRemaining:
              cookies.get('form-optionTheoretical-hoursRemaining') || 0,
    ...
    chartDimensions() {
      const [chartResolution, chartCenter, chartHalfWidth] = Object.values(
        this.state.debug
      ).map(parseFloat);
      return {
    ...
    pnl() {
      ...
      const [volatilityChange, hoursPassed] = Object.values(
        this.state.analysis
      ).map(parseFloat);
    ...
    levels() {
      return this.state.positions.options
    ...
    probs() {
      const [currentPrice, currentVolatility] = Object.values(
        this.state.analysis
      )
        .slice(2)
        .map(parseFloat);
      ...
      const prob = (level) =>
        compute_prob_itm(currentPrice, level, currentVolatility, time);
          ...
          <div className="w3-cell-row w3-container">
            <div className="w3-cell">
              ...
              <Positions
                ...
                title="Underlying"
                posToString={(pos) => {
                  const hedgeOnly = !pos.hedgeOnly ? (
                    ''
                  ) : (
                    <small>(hedge only)</small>
                  );
                  return (
                    <span>
                      {pos.quantity}x {pos.price} {hedgeOnly}
                    </span>
                  );
                }}
                togglePosition={(e, i) =>
                  this.togglePosition(e, 'underlying', i)
                }
                removePosition={(e, i) =>
                  this.removePosition(e, 'underlying', i)
                }
              />
              <Positions
            ...
            <div className="w3-cell">
              <PnlChart
                ...
                levels={this.levels()}
                zero={true}
                currentPrice={this.state.analysis.currentPrice}
              />
              <Analysis
                analysis={analysis}
  ...
  return (
    <form>
      <div className="w3-section w3-theme-d5">
        ...
        <InputPrice
          value={price}
          onChange={(e) => onFormChange(e, formStateName, 'price')}
        />
        <Row>
    ...
    <form>
      <div className="w3-section w3-theme-dark">
        ...
        <LabeledRow label="Implied volatility">
          <div className="my-cell w3-cell-middle">{toPercent(iv)}</div>
        </LabeledRow>
      </div>
      <Submit onLong={onLong} onShort={onShort} />
    </form>
        ...
        <LabeledRow label="Type">
          <div style={{ paddingBottom: '6px' }}>
            <div>
              ...
              />{' '}
              <label htmlFor="call">
                call ${round_to(call, 2)} ({round_to(call / underlyingPrice, 4)}{' '}
                BTC)
              </label>
            </div>
            <div>
              ...
              />{' '}
              <label htmlFor="put">
                put ${round_to(put, 2)} ({round_to(put / underlyingPrice, 4)}{' '}
                BTC)
              </label>
            </div>
          </div>
...
function InputVolatility(props) {
  return (
    <Input label="Volatility" type="number" min="0.01" step="0.01" {...props} />
  );
}
...
function Submit(props) {
  ...
  return (
    <div className="w3-cell-row w3-card">
      <div className="w3-cell">
        <button
          type="button"
          onClick={onLong}
...
function PnlChart(props) {
  ...
  return (
    <div className="w3-margin w3-container w3-theme-dark w3-padding-24">
      ...
      <ListBlock>
        ...
        <li>
          <LabeledRow label="Current volatility">
            <input
              className="w3-input"
              value={analysis.currentVolatility}
...
function pnl_underlying(x, quantity, entry, hedgeOnly) {
  if (
    hedgeOnly &&
    ((quantity < 0 && x > entry) || (quantity > 0 && x < entry))
  ) {
    return 0;
  } else {
