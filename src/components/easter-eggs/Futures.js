...
const Row = ({ left, right }) => (
  <div className="w3-cell-row">
    <div className="w3-cell" style={{ width: '160px' }}>
      {left}
    </div>
...
export default function Futures() {
  const [future, setFuture] = useLocal(futureId, { initialValue: NaN });
  const [spot, setSpot] = useLocal(spotId, { initialValue: NaN });
  const [days, setDays] = useLocal(daysId, { initialValue: NaN });
  const [hours, setHours] = useLocal(hoursId, { initialValue: NaN });
  const [mins, setMins] = useLocal(minsId, { initialValue: NaN });
  const [annual, setAnnual] = useLocal(annualId, { initialValue: 365 });
  const [spotTarget, setSpotTarget] = useLocal(spotTargetId, {
    initialValue: NaN,
  });
  const [futureTarget, setFutureTarget] = useLocal(futureTargetId, {
    initialValue: NaN,
  });
  ...
  const [
    ...
    fspotTarget,
    ffutureTarget,
  ] = [future, spot, annual, days, hours, mins, spotTarget, futureTarget].map(
    parseFloat
  );
  const premium =
    ((ffuture / fspot - 1) * 1440 * fannual) /
    (1440 * fdays + 60 * fhours + fmins);
  const future_equiv = (fspotTarget * ffuture) / fspot;
  const spot_equiv = (ffutureTarget * fspot) / ffuture;

  return (
    <form className="w3-container" style={{ width: '275px' }}>
