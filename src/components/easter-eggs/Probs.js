...
import { probs, reverse } from '../../util/math.bs';
...
class Probs extends Component {
  ...
  constructor(props) {
    ...
    this.autoVolatilityChangeHandler = this.autoVolatilityChangeHandler.bind(
      this
    );
  ...
  componentDidMount() {
    ...
    this.updateInterval = setInterval(() => {
      Promise.all([
        fetch(`${DATA_URI}/data/price.txt`).then((response) => response.text()),
        fetch(`${DATA_URI}/data/historical-volatility.txt`).then((response) =>
          response.text()
        ),
      ]).then((result) => {
  ...
  render() {
    ...
    if (!price) {
      ...
      let touch, itm, rev_itm, rev_touch, direction;
      if (time > 0 && target && volatility) {
        ({ itm, touch } = probs(price, target, volatility, time));
        rev_itm = reverse(itm, target, volatility, time);
        rev_touch = reverse(touch, target, volatility, time);
      ...
      return (
        <div className="w3-container">
          ...
          <p>
            ...
            <input
              id="days"
              onChange={this.daysChangeHandler}
              size="1"
              value={days ? days : 0}
            />{' '}
            days and{' '}
          ...
          {!probs ? (
            ...
            <div>
              ...
              <p>
                <b>
                  {round_to(100 * itm)}% chance to close {direction}{' '}
                  {target.toLocaleString()}
                </b>
              </p>
              <p>
                {round_to(100 * itm)}% chance to touch (
                {rev_itm
              ...
              <p>
                <b>
                  {round_to(100 * touch)}% chance to touch{' '}
                  {target.toLocaleString()}
                </b>
              </p>
              <p>
                {round_to(100 * touch)}% chance of closing {direction} (
                {rev_touch.map((price) => Math.round(price)).join(', ')}).
              </p>
            </div>
          )}
        </div>
